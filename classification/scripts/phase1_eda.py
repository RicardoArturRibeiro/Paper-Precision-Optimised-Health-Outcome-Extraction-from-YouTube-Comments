...
def load_corpus_robust(filepath):
    ...
    strategies = [
        # Strategy 1: Standard read with Python engine (more flexible)
        {
        ...
        # Strategy 3: Python engine with different encoding
        {
        ...
        # Strategy 4: Escape character handling
        {
            'name': 'Python engine with escape character',
            'params': {
                'engine': 'python',
                'on_bad_lines': 'skip',
                'encoding': 'utf-8',
    ...
    for i, strategy in enumerate(strategies, 1):
        try:
            ...
            df = pd.read_csv(filepath, low_memory=False, **strategy['params'])
...
with pd.ExcelWriter(summary_file, engine='openpyxl') as writer:
    ...
    if 'channel_profile' in dir():
        channel_profile.to_excel(writer, sheet_name='Channel_Profile', index=False)
